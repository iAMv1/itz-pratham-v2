...
import type { Process as ProcessContent } from "@/content/site";
...
export function Process({ process }: { process: ProcessContent }) {
  ...
  const [blocks, setBlocks] = useState<Block[]>(
    () => process.steps.map((s) => ({ num: s.num, cmd: s.cmd, out: s.out, done: false }))
  );
    ...
    const io = new IntersectionObserver(
      (entries) => {
        for (const e of entries) {
          ...
          if (idx >= 0) setActiveStep(Math.min(idx, process.steps.length - 1));
        }
      },
  ...
  return (
    <section id="process" className="relative border-y-2 border-ink bg-paper px-[clamp(20px,4vw,48px)] py-[clamp(64px,8vh,110px)]">
      ...
      <div className="relative z-10">
        ...
        <div className="mt-10 grid grid-cols-1 items-start gap-[clamp(24px,3vw,48px)] lg:grid-cols-[1.55fr_1fr]">
          ...
          <div className="border-2 border-ink bg-paper-2 shadow-[8px_8px_0_0_#1D5B9E]">
            <div className="flex items-center gap-2 border-b-2 border-ink px-4 py-3">
              ...
              <span className="ml-auto font-mono text-xs text-ink/70">{process.title}</span>
            </div>
        ...
        {/* measured — real numbers from a Lighthouse run, auto-refreshed per deploy */}
        <div id="perf" className="mt-14 border-2 border-ink bg-paper-2 p-[clamp(18px,2.5vw,28px)] shadow-hard">
          ...
          <div className="mt-4 grid grid-cols-1 gap-px border-2 border-ink bg-ink sm:grid-cols-3 lg:grid-cols-6">
            {[
              ...
              { k: "SEO", v: String(performanceData.scores.seo) },
              { k: "CLS", v: performanceData.metrics.cls },
              { k: "FCP", v: performanceData.metrics.fcp },
              { k: "TBT", v: performanceData.metrics.tbt },
            ].map((m) => (
          ...
          <p className="mt-3 max-w-[70ch] font-mono text-[11.5px] leading-relaxed tracking-wider text-muted-foreground">
            <span className="text-cobalt">▸</span> LCP {performanceData.metrics.lcp} includes the cinematic curtain (plays once per session, skipped on repeat visits and deep links), by
            design — the preloader is the site&apos;s signature moment and is skipped on repeat visits and deep links.
            Regenerated by <code className="bg-paper-2 px-1">scripts/measure.mjs</code> on every deploy — these numbers
            come from a Lighthouse run against the live site, not markup.
          </p>
        </div>
